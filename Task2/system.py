...
class LibrarySystem:
    ...
    def popular_book(self, n=5):
        """Return top N most popular books using Heap + Heap Sort"""
        from heap import heap_sort
        
        book_list = []
        for book in self.books:
            # Simple popularity: borrowed books get higher score
            borrow_count = 10 if book.is_borrowed else 0
            book_list.append((book, borrow_count))
        
        if not book_list:
            return []
        
        sorted_books = heap_sort(book_list)
