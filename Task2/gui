...
class LibraryGUI:
    ...
    def show_login_screen(self):
        ...
        tk.Label(frame, text="Admin IDs start with 'a'\nStudent IDs start with 's'", fg="gray").pack()
    ...
    def show_dashboard(self):
        ...
        tk.Button(btn_frame, text="Most Popular Books", command=self.show_popular_books, bg="#4CAF50", fg="white").pack(side="left", padx=5)
    ...
    def show_popular_books(self):
        popular_list = self.library.popular_book(n=5)
        if not popular_list:
            messagebox.showinfo("Most Popular Books", "No books available.")
            return

        msg = "Most Popular Book\n"
        for i, book in enumerate(popular_list, 1):
            status = "Borrowed" if book.is_borrowed else "Available"
            msg += f"{i}. {book.title} by {book.author} ({book.book_id}) - {status}\n"

        messagebox.showinfo("Most Popular Book", msg)
